...
describe('13 - Renderize um `select` do formulário em `<AddMovie />` para selecionar o gênero do novo filme', () => {
  ...
  it.only('Renderize um select com 3 opções de genero de filme', () => {
    expect(genreInput).toBeInTheDocument();
    expect(genreOptions).toHaveLength(options.length);
  });

  it.only('Será validado se o component renderiza a label "Gênero" para o select de gênero', () => {
    expect(genreInputLabel).toBeInTheDocument();
    expect(genreInputLabel).toHaveTextContent('Gênero');
  });

  it.only('Será validado se todas as opções no select tem o texto e o valor esperados, que são, respectivamente: Ação e action, Comédia e comedy, Suspense e thriller', () => {
  ...
  it.only('Será validado se o gênero selecionado inicialmente é o "action"', () => {
